...
Codebit:    # a chunk of code parsed from earlier stage
    ...
    append(magic vars)          # (deprecated, inject smart enough) appends another "slice" of vars into the codebit (error will be thrown if injection inteface is violated)
    ...
    generate()                  # produces output for this codebit, if there are missing variables, will produce another template with those variables left
    finalize()                  # like generate but
...
Stage:      # set of directories that define compilation steps for this stage
    get(name)                   # get a codebit with this name
